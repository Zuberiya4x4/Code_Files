...
class EnhancedPDFHeaderExtractor:
    ...
    def extract_json_data(self, field_data: Dict) -> Dict[str, Any]:
        """Enhanced JSON extraction with metadata and coordinates"""
        json_data = {}
        metadata = {
        ...
        for field_name, data in field_data.items():
            ...
            # Extract coordinates
            header_bbox = data.get("header", {}).get("bbox", [])
            value_bboxes = [value.get("bbox", []) for value in data.get("values", [])]
    ...
    def save_coordinates_to_text(self, field_data: Dict, output_dir: str = "coordinates_output") -> str:
        """Save field coordinates to a text file in a structured format"""
        os.makedirs(output_dir, exist_ok=True)
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = os.path.join(output_dir, f"field_coordinates_{timestamp}.txt")
        
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("FIELD COORDINATES EXTRACTION REPORT\n")
            f.write("="*50 + "\n")
            ...
            for field_name, data in field_data.items():
                ...
                f.write("-"*30 + "\n")
                f.write(f"Value: {data.get('clean_value', '')}\n")
                f.write(f"Confidence: {data.get('confidence', 0)}%\n\n")
                
                # Header bbox
                header_bbox = data.get("header", {}).get("bbox", [])
                if header_bbox:
                    f.write("Header Bounding Box:\n")
                    f.write(f"  (x0, y0, x1, y1): {header_bbox}\n\n")
                
                # Value bboxes
                value_bboxes = [value.get("bbox", []) for value in data.get("values", [])]
                if value_bboxes:
                    f.write("Value Bounding Box(es):\n")
                    for i, bbox in enumerate(value_bboxes):
                        f.write(f"  Value {i+1}: (x0, y0, x1, y1): {bbox}\n")
                    f.write("\n")
                
                # Combined bbox
                combined_bbox = data.get("combined_bbox", [])
                if combined_bbox:
                    f.write("Combined Bounding Box (Header + Values):\n")
                    f.write(f"  (x0, y0, x1, y1): {combined_bbox}\n\n")
...
def main():
    ...
    if 'coordinates_file' not in st.session_state:
        st.session_state.coordinates_file = None
    ...
    if uploaded_file is not None:
        ...
        if not st.session_state.processing_complete:
            ...
            coordinates_file = extractor.save_coordinates_to_text(field_data)
            ...
            st.session_state.coordinates_file = coordinates_file
    ...
    if st.session_state.processing_complete:
            ...
            for field_name, data in st.session_state.field_data.items():
                ...
                with st.expander(f"📌 {field_name.replace('_', ' ').title()}", expanded=True):
                    ...
                    with col_info:
                        ...
                        st.write(f"**Header Coordinates:** {data.get('header', {}).get('bbox', 'N/A')}")
                        st.write(f"**Value Coordinates:** {[v.get('bbox', 'N/A') for v in data.get('values', [])]}")
        ...
        st.divider()
        col_submit, col_download_json, col_download_coords, col_stats = st.columns([2, 1, 1, 1])
            ...
            if st.button("🚀 Submit & Generate JSON", type="primary", use_container_width=True):
                ...
                for field_name, status in st.session_state.field_status.items():
                    if status == "accepted" and field_name in st.session_state.field_data:
                        field_info = st.session_state.field_data[field_name]
                        final_json[field_name] = {
                            "value": field_info['clean_value'],
                            "confidence": field_info['confidence'],
                            "coordinates": {
                                "header_bbox": field_info.get('header', {}).get('bbox', []),
                                "value_bboxes": [v.get('bbox', []) for v in field_info.get('values', [])],
                                "combined_bbox": field_info.get('combined_bbox', [])
                            }
                        }
        ...
        with col_download_coords:
            if st.session_state.coordinates_file:
                with open(st.session_state.coordinates_file, 'rb') as f:
                    coordinates_data = f.read()
                
                st.download_button(
                    label="📥 Download Coordinates",
                    data=coordinates_data,
                    file_name=f"field_coordinates_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                    mime="text/plain",
                    use_container_width=True
                )
        ...
        if st.session_state.coordinates_file:
            with open(st.session_state.coordinates_file, 'r', encoding='utf-8') as f:
                coordinates_content = f.read()
            
            st.text_area("Coordinates File Content", coordinates_content, height=300)
        else:
            st.info("No coordinates file available")
    ...
    with st.expander("ℹ️ How to use this tool", expanded=False):
        ...
        5. **Download** the JSON or coordinates file
        ...
        - Header bounding box coordinates (x0, y0, x1, y1)
        - Value bounding box coordinates for each value
        - Combined bounding box coordinates (header + values)
        """)
