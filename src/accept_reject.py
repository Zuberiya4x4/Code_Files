# bounding boxes done detected fields are working wrong for this code
...
class PDFHeaderExtractor:
    def __init__(self):
        ...
        self.header_patterns = {
            ...
            "advice_ref": [r"advice\s*reference\s*no", r"advice\s*ref\s*no", r"advice\s*reference", r"advice\s*ref", r"ref\s*no"],
            "receipt_name": [r"recipient'?s?\s*name", r"company\s*name", r"beneficiary\s*name"],
            ...
            "remitter_to_beneficiary_info": [r"remitter\s*to\s*beneficiary", r"additional\s*info", r"payment\s*info"]
        }
    ...
    def find_values_for_headers(self, text_blocks: List[Dict], header_matches: Dict) -> Dict[str, Dict]:
        ...
        for field_name, headers in header_matches.items():
            ...
            for header in headers:
                ...
                colon_pattern = header["text"] + r"\s*:\s*(.+)"
                for block in text_blocks:
                    if re.search(re.escape(header["text"]) + r"\s*:\s*(.+)", block["text"], re.IGNORECASE):
                        match = re.search(re.escape(header["text"]) + r"\s*:\s*(.+)", block["text"], re.IGNORECASE)
                        if match:
                            value_text = match.group(1).strip()
                            if value_text and len(value_text) > 2:
                                # Create a virtual value block
                                value_start_x = header_bbox[0] + len(header["text"]) * (header["font_size"] * 0.6)
                                virtual_value = {
                                    "text": value_text,
                ...
                # If no colon pattern found, look for adjacent values
                if best_score == float('inf'):
                    potential_values = []
                    
                    for block in text_blocks:
                        # Skip if it's the header itself or contains the header text
                        if (block["text"].lower() == header["text"].lower() or 
                            header["text"].lower() in block["text"].lower()):
                            continue
                        ...
                        # Calculate distances
                        horizontal_distance = abs(block_x - header_x_end)
                        vertical_distance = abs(block_y - header_y)
                        
                        # Check if it's to the right of the header (same line)
                        if vertical_distance < 10 and block_x > header_x_end:
                        ...
                        # Check if it's below the header (next line)
                        elif block_y > header_y and vertical_distance < 50:
                            # Prefer values that are somewhat aligned horizontally
                            horizontal_alignment_penalty = abs(block_x - header_bbox[0]) * 0.5
                            score = vertical_distance + horizontal_alignment_penalty
                            potential_values.append({
                                "block": block,
                                "score": score,
                                "type": "below"
                    ...
                    if potential_values:
                        ...
                        closest_value = potential_values[0]
                        
                        if closest_value["score"] < best_score:
                            best_match = header
                            best_value = closest_value["block"]
                            best_score = closest_value["score"]
            
            if best_match and best_value:
                # Create combined bounding box for header + value
                header_bbox = best_match["bbox"]
                value_bbox = best_value["bbox"]
                
                combined_bbox = [
                    min(header_bbox[0], value_bbox[0]),
                    min(header_bbox[1], value_bbox[1]),
                    max(header_bbox[2], value_bbox[2]),
                    max(header_bbox[3], value_bbox[3])
                ]
                
                # Clean up the extracted value text
                value_text = best_value["text"].strip()
                # Remove any remaining header text from the value
                if best_match["text"].lower() in value_text.lower():
                    value_text = re.sub(re.escape(best_match["text"]), "", value_text, flags=re.IGNORECASE).strip()
                ...
                field_data[field_name] = {
                    ...
                    "extracted_text": f"{best_match['text']}: {value_text}",
