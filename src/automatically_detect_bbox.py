...
# ---------------- State ----------------
for key in ["boxes", "json_table", "selected_page_index", "highlight_box_idx", "table_view_mode", "update_flag"]:
    if key not in st.session_state:
        st.session_state[key] = [] if key == "boxes" else {} if key == "json_table" else 0 if key in ["selected_page_index", "highlight_box_idx"] else "row-wise" if key == "table_view_mode" else False
...
if uploaded_file:
    ...
    # Update annotated image after any edits
